...
import { Search, Lock, ShieldCheck, RefreshCw, CheckCircle2, AlertCircle, Key, FileText, Zap, Loader2, Shield, Clock, Database, Cpu, TrendingUp, Lightbulb, X } from "lucide-react";
...
import { getRandomFact, getRandomQuote, fheFacts } from "@/lib/fheFacts";
...
export default function EncryptedSearchV2() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchResults.length > 0) {
        ...
        toast.info("No matches found. Try a different search term.");
      }
    } catch (error) {
      console.error("Search error:", error);
      toast.error("Homomorphic search failed. Please try again.");
    } finally {
      ...
      <div className="space-y-8">
        ...
        {showFacts && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="p-4 md:p-6 rounded-2xl bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 space-y-4"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4 flex-1">
                <div className="text-3xl">{currentFact.icon}</div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-black text-sm md:text-base mb-1">{currentFact.title}</h4>
                  <p className="text-xs md:text-sm text-muted-foreground leading-relaxed">{currentFact.description}</p>
                  <Badge className="mt-3 bg-purple-500/20 text-purple-500 border-purple-500/30 font-black text-[10px] capitalize">
                  ...
                  >
                    ...
                    {results.length > 0 ? (
                      ...
                      <div className="text-center py-8 space-y-2">
                        ...
                        <p className="text-xs text-muted-foreground">Try searching for a different term or upload another PDF.</p>
                      </div>
                    )}
